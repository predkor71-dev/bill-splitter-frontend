...
function App() {
  ...
  const [myGroups, setMyGroups] = useState([]); const [currentGroup, setCurrentGroup] = useState(null)
  const [expenses, setExpenses] = useState([]); const [balances, setBalances] = useState(null)
  ...
  const [email, setEmail] = useState(''); const [pass, setPass] = useState(''); const [regName, setRegName] = useState(''); const [phone, setPhone] = useState('')
  const [isRegister, setIsRegister] = useState(false)
  const [inviteEmail, setInviteEmail] = useState(''); const [newGroupTitle, setNewGroupTitle] = useState('')
  ...
  const fetchMyGroups = async () => {
    if (!user) return
    const res = await fetch(`http://127.0.0.1:8000/api/my-groups/${user.id}`)
  ...
  const fetchGroupData = async () => {
    ...
    try {
        const resExp = await fetch(`http://127.0.0.1:8000/api/expenses/list/${currentGroup.id}`)
        const dExp = await resExp.json(); if (dExp.status === 'success') setExpenses(dExp.data)

        const resBal = await fetch(`http://127.0.0.1:8000/api/balance/${currentGroup.id}`)
        const dBal = await resBal.json(); if (dBal.status === 'success') setBalances(dBal)

        const resMem = await fetch(`http://127.0.0.1:8000/api/groups/${currentGroup.id}/members`)
        const dMem = await resMem.json(); if (dMem.status === 'success') setGroupMembers(dMem.data)

        const resDebts = await fetch(`http://127.0.0.1:8000/api/debts/${currentGroup.id}`)
        ...
        const resStats = await fetch(`http://127.0.0.1:8000/api/stats/${currentGroup.id}`)
...
function App() {
  ...
  const fetchGroupData = async () => {
    ...
    try {
        ...
        const resUserStats = await fetch(`http://127.0.0.1:8000/api/stats/users/${currentGroup.id}`)
        setUserStats(await resUserStats.json())

        const resDateStats = await fetch(`http://127.0.0.1:8000/api/stats/dates/${currentGroup.id}`)
  ...
  const handleDeleteGroup = async (groupId, title) => {
    if (window.confirm(`Вы уверены, что хотите НАВСЕГДА удалить группу "${title}" и все финансовые записи внутри нее?`)) {
      const res = await fetch(`http://127.0.0.1:8000/api/groups/delete/${groupId}`, { method: 'DELETE' })
      if (res.ok) {
  ...
  const handleRemoveMember = async (memberId, memberName) => {
    if (window.confirm(`Вы уверены, что хотите исключить ${memberName} из группы?`)) {
      const res = await fetch(`http://127.0.0.1:8000/api/groups/${currentGroup.id}/members/${memberId}`, { method: 'DELETE' })
      if (res.ok) {
        fetchGroupData();
      }
    }
  ...
  const handleSubmitExpense = async (e) => {
    ...
    const url = editId ? `http://127.0.0.1:8000/api/expenses/update/${editId}` : 'http://127.0.0.1:8000/api/expenses/add'
  ...
  const deleteExpense = async (id) => {
    if (window.confirm("Удалить этот расход навсегда?")) {
        const res = await fetch(`http://127.0.0.1:8000/api/expenses/delete/${id}`, { method: 'DELETE' })
  ...
  const acceptSettlement = async (id) => {
    await fetch(`http://127.0.0.1:8000/api/settlements/${id}/accept`, { method: 'POST' })
    fetchGroupData()
  }
  ...
  if (!currentGroup) return (
    <div className="app-container" style={{maxWidth: '800px'}}>
      ...
      <div className="card" style={{marginBottom: '32px', background: '#f8fafc'}}>
        ...
        <form onSubmit={async (e)=>{e.preventDefault(); await fetch('http://127.0.0.1:8000/api/groups', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({title:newGroupTitle, creator_id:user.id})}); setNewGroupTitle(''); fetchMyGroups()}} style={{display:'flex', gap:12}}>
    ...
    <div className="app-container" style={{position: 'relative'}}>
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:30, borderBottom: '1px solid #edf2f7', paddingBottom: '20px', gap: '20px', flexWrap: 'wrap'}}>
        ...
        <div style={{display:'flex', alignItems: 'center', gap:16}}>
          ...
          <button onClick={()=>{setUser(null); localStorage.removeItem('user')}} className="btn-secondary" style={{width:'auto', padding: '10px 16px'}}>Выйти</button>
        </div>
      </div>
      ...
      {activeTab === 'analytics' ? (
        ...
        <div className="main-grid">
                      ...
                      return (
                        <label key={m.id} className={`custom-checkbox-label ${isSelected ? 'selected' : ''}`}>
                          <input
                            ...
                            checked={isSelected}
                            onChange={()=>setSelectedMembers(p => p.includes(m.id) ? p.filter(x=>x!==m.id) : [...p, m.id])}
                          />
                          <div className="custom-checkbox-box"></div>
                          <span style={{fontSize: 14, color: isSelected ? '#2b6cb0' : '#4a5568', fontWeight: isSelected ? '600' : '400'}}>
                            {m.name} {m.id === user.id && <small style={{color: '#a0aec0', fontWeight: 'normal'}}>(вы)</small>}
                          </span>
                        </label>
                      )
                    })
            ...
            <div className="card" style={{maxHeight: '400px', overflowY: 'auto', padding: '12px 20px'}}>
              ...
              {expenses.map(item => (
                <div key={item.id} className="expense-item">
                  <div>
                    <span style={{fontWeight: '600', fontSize: '15px'}}>{item.category.split(' ')[0]} {item.description}</span>
                    <br/>
                    <small style={{color: '#718096'}}>Оплатил: <b>{item.paid_by}</b></small>
                  </div>
                  <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
                    <b style={{fontSize: '16px', color: '#1a202c'}}>{item.amount} ₽</b>
                    <div style={{display: 'flex', gap: '8px'}}>
                      <span onClick={()=>startEdit(item)} style={{cursor:'pointer', fontSize: '16px', padding: '4px'}}>✏️</span>
                      <span onClick={()=>deleteExpense(item.id)} style={{cursor:'pointer', fontSize: '16px', padding: '4px'}}>🗑️</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="right">
                ...
                return (
                  <div key={u.name} className={`balance-row ${isPositive ? 'positive-border' : 'negative-border'}`}>
                    ...
                    <span className={isPositive ? 'positive' : 'negative'}>
                      {isPositive ? `+${u.balance}` : u.balance} ₽
                    </span>
                  </div>
                );
              })}
            ...
            <div className="card">
              {debts.transactions.length === 0 && debts.pending.length === 0 ? (
                <p style={{color:'#38a169', fontWeight:'600', textAlign: 'center', margin: '10px 0'}}>🎉 Все взаиморасчеты закрыты!</p>
              ) : null}

              {debts.pending.map(p => (
                <div key={p.id} className="balance-row" style={{background: '#fffaf0', borderLeft: '4px solid #dd6b20', padding: '12px 16px'}}>
                  <div style={{fontSize: '14px'}}>
                    <strong>{p.from_name}</strong> отправил <strong>{p.to_name}</strong>
                    <br/>
                    <b style={{color: '#dd6b20'}}>{p.amount} ₽</b>
                  </div>
                  <div>
                    {user.id === p.to_id ? (
                      <button onClick={() => acceptSettlement(p.id)} style={{background: '#38a169', padding: '6px 12px', fontSize: '13px', width: 'auto'}}>✅ Подтвердить</button>
                    ) : (
                      <span style={{color: '#dd6b20', fontSize: '13px', fontWeight: '500'}}>⏳ Проверка...</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              {debts.transactions.map((t, idx) => {
                ...
                return (
                  <div key={idx} className="balance-row" style={{background: '#f8fafc', padding: '14px 16px'}}>
                    <div style={{fontSize: '14px'}}>
                      <strong>{t.from_name}</strong> ➡️ <strong>{t.to_name}</strong>
                      <br/>
                      <small style={{color: '#718096'}}>Сумма долга: <b>{t.amount} ₽</b></small>
                    </div>
                    <div style={{display:'flex', gap:6, alignItems:'center'}}>
                      <button onClick={()=>setQrData(t)} style={{padding: '8px', background:'#805ad5', width:'36px', height: '36px', margin:0, fontSize: '16px'}}>📱</button>
                      {user.id === t.from_id && (
                        <form onSubmit={(e) => { e.preventDefault(); sendSettlement(t.to_id, e.target.amount.value); }} style={{display:'flex', gap:6, margin:0}}>
                          <input type="number" name="amount" defaultValue={t.amount} max={t.amount} step="0.01" required style={{width: '70px', padding: '8px', margin:0, height: '36px', borderRadius: '8px'}}/>
                          <button type="submit" style={{padding: '0 12px', margin:0, background:'#3182ce', height: '36px', fontSize: '13px', width: 'auto'}}>Отдать</button>
                        </form>
                      )}
                    </div>
                  </div>
            ...
            <div className="card" style={{padding: '20px'}}>
              ...
              <div style={{marginBottom: '16px', maxHeight: '180px', overflowY: 'auto'}} className="members-list-scroll">
                {groupMembers.map(m => (
                  <div key={m.id} className="member-row">
                    <span style={{fontSize: '15px', color: '#2d3748', fontWeight: m.id === user.id ? '600' : '400'}}>
                      {m.name} {m.id === user.id ? <span style={{color: '#718096', fontWeight: 'normal'}}>(Вы)</span> : ''}
                    </span>
                    {m.id !== user.id && (
                      <span
                        onClick={() => handleRemoveMember(m.id, m.name)}
                        style={{cursor: 'pointer', fontSize: '14px', color: '#e53e3e', padding: '4px'}}
              ...
              <div style={{borderTop: '1px solid #e2e8f0', paddingTop: '16px'}}>
                <form onSubmit={async (e)=>{e.preventDefault(); await fetch('http://127.0.0.1:8000/api/groups/add_member', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({group_id:currentGroup.id, email:inviteEmail})}); setInviteEmail(''); fetchGroupData()}} style={{display:'flex', gap:8}}>
