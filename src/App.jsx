...
function App() {
  ...
  const fetchMyGroups = async () => {
    ...
    if (data.status === 'success') setMyGroups(data.data);
  };
  ...
  const handleRemoveMember = async (memberId, memberName) => {
    if (window.confirm(`Исключить ${memberName} из группы?`)) {
  ...
  const handleSubmitExpense = async (e) => {
    ...
    if (res.ok) { setEditId(null); setAmount(''); setDesc(''); setCategory('Другое'); setSplitMode('equal'); setExactAmounts({}); fetchGroupData(); }
  };
  ...
  if (!user) return (
    <div className="app-container" style={{maxWidth: '450px', marginTop: '100px'}}>
      ...
      <p onClick={()=>setIsRegister(!isRegister)} style={{cursor:'pointer', color:'#3182ce', textAlign: 'center'}}>{isRegister ? 'Уже есть аккаунт? Войти' : 'Нет аккаунта? Регистрация'}</p>
    </div>
  );
