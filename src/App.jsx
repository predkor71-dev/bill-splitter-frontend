...
function App() {
  ...
  // Использование API_URL из Vercel ENV
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:20}}>
        {myGroups.map(g => (
          <div key={g.id} className="card" style={{position: 'relative', border:'1px solid #e2e8f0', padding: '30px 20px', margin: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px'}}>
            ...
            <span onClick={(e) => { e.stopPropagation(); handleDeleteGroup(g.id, g.title); }} style={{position: 'absolute', top: '12px', right: '12px', cursor: 'pointer', opacity: 0.4, fontSize: '14px', padding: '4px'}} title="Удалить группу">❌</span>
          </div>
        ))}
      </div>
    </div>
          ...
          <div className="card" style={{textAlign:'center', width: '340px', padding: '30px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)'}}>
            ...
            {qrData.to_phone ? (
              <div style={{marginBottom: '24px', background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
                <p style={{margin: '0 0 12px 0', fontSize: '15px', color: '#4a5568'}}>Номер привязан к СБП: <br/><b style={{color: '#1a202c', fontSize: '16px'}}>{qrData.to_phone}</b></p>
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px'}}>
            <div className="card" style={{margin: 0}}>
              ...
              <div style={{height: 300}}>
                <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.filter(s => s.value > 0)} cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={4} dataKey="value">{stats.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value) => `${value} ₽`} /><Legend iconType="circle" /></PieChart></ResponsiveContainer>
              </div>
            </div>
            <div className="card" style={{margin: 0}}>
              <h3 style={{fontSize: '16px', marginBottom: '20px', color: '#4a5568'}}>💳 Общие расходы участников</h3>
              <div style={{height: 300}}>
                <ResponsiveContainer width="100%" height="100%"><BarChart data={userStats}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="name" stroke="#718096" fontSize={12} /><YAxis stroke="#718096" fontSize={12} /><Tooltip formatter={(value) => `${value} ₽`} /><Bar dataKey="amount" fill="#3182ce" radius={[6, 6, 0, 0]} name="Оплачено" /></BarChart></ResponsiveContainer>
              </div>
            </div>
          </div>
          <div className="card" style={{margin: 0}}>
            ...
            <div style={{height: 300}}>
              <ResponsiveContainer width="100%" height="100%"><LineChart data={dateStats}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="date" stroke="#718096" fontSize={12} /><YAxis stroke="#718096" fontSize={12} /><Tooltip formatter={(value) => `${value} ₽`} /><Line type="monotone" dataKey="amount" stroke="#38a169" name="Сумма" strokeWidth={3} activeDot={{ r: 6 }} /></LineChart></ResponsiveContainer>
            </div>
          </div>
        </div>
      ) : (
